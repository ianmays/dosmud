...
Fixtures call `game_reset_fixture_baseline` first (same mutable fields as `game_init`: mode, room, tick, player stats, bag, combat, NPC roster state, enemy and player corpses, ground items, env focus, env interact menu, global weather (`weather_kind`, `weather_expires_tick`), global day/night (`day_phase`, `day_expires_tick`, `night_lost`), and map exploration). That leaves the world graph and `GameState.seed` unchanged. `main.c` then calls `plat_seed_rng(game.seed)` so tracked `plat_rand()` draws match that seed and follow-up rolls do not depend on earlier commands in the same run.
...
| `player_defeat_ready` | Road combat at 1 HP with level-4 cumulative XP, berry/herb/marsh-root bag, spear equipped, active winning bandit, quiet ticks, and injected attack/enemy-damage rolls for a deterministic lethal reply |
...
| `frog_dialogue` / `watchman_dialogue` / `herbalist_dialogue` / `archivist_dialogue` | Same rooms with the fixed room-NPC dialogue already open and rendered (use for modal-policy snapshots without walking through `talk`) |
...
| `corpse_loot_weather_rain` | `weather_rain_ready` plus a one-item enemy corpse at camp; proves quiet `loot` does not advance the pending rain transition ([#206](https://github.com/ianmays/dosmud/issues/206)) |
| `weather_fog` | Camp with `GAME_WEATHER_FOG` active for room-look weather footer phrase |
| `night_at_camp` | Camp at night (`GAME_NIGHT`), `night_lost` clear, for room-look night footer phrase ([#130](https://github.com/ianmays/dosmud/issues/130)) |
...
| `env_clues_rustle_water` | Sets both the rustle and water bits on the current room, for sequential single-argument `inspect` and compact `look` footer snapshots |
...
**Replay sidecar (`replay_log`):** the only snapshot that also golden-checks a `--replay-log` sidecar file. `--replay-log` is a regular feature in every build; the snapshot runs against the `TEST_MODE` binary from `make test` only because that is the binary `make snapshot-run` uses, not because logging is test-only. `make snapshot-run` runs `./dosmud --seed 1234 --replay-log tests/regression/replay_log_log.output < tests/regression/replay_log.input` and diffs both stdout (`replay_log.expect`) and the sidecar log (`replay_log_log.expect`). Player-visible stdout stays unchanged; the log records `dosmud-replay-v2` header lines plus per-step metadata and serialized `GameEvent` rows (`arg0`..`arg3`, `value0`, `room`, `room_items`, `text`).

**Save sidecar (`save_load`, `herbalist_save_load`, `player_defeat_recovery`):** exercises the in-session `save` / `load` shell commands. `make snapshot-run` removes `save.dat`, runs the snapshot, diffs stdout against the matching `.expect`, then removes `save.dat` again so the single-slot file does not leak across tests.

**Version snapshots (`version`, `version_cli`):** build identity includes generated Git metadata, so the expectation files keep `@VERSION@` placeholders. `make snapshot-run` substitutes the current `BUILD_VERSION_STRING` from `build/include/version.h` before diffing stdout and fails if either `.expect` file drops the `@VERSION@` token.
...
**NPC talk:** `frog_hint`, `frog_replies`, `watchman_talk`, `watchman_warned_followup`, `watchman_meal_peckish`, `watchman_meal_tower_desc`, `traveler_replies`, `traveler_talk_blocked`, `lost_animal_replies`, `lost_animal_talk_blocked`, `peddler_replies`, `peddler_talk_blocked`, `frog_modal_policy`, `watchman_modal_policy`, `herbalist_modal_policy`, `archivist_modal_policy`, `herbalist_talk`, `herbalist_request`, `herbalist_incomplete`, `herbalist_complete`, `herbalist_followup`, `herbalist_save_load`, `herbalist_give_reject`, `herbalist_give_floor`, `archivist_talk`, `talk_nobody`, `dialogue_menu_exit`, `game_event_dialogue`, `narrative_indirection` (`frog_hint` proves the generic room-NPC hint also applies at the pond; the Herbalist set covers the first authored narrative slice plus the reusable room-NPC exchange follow-up through request (including REQUESTED root menu and gossip-leaves-dialogue), reminder, accepted hand-in, bag-full floor reward, reject feedback, save/load (`herbalist_menu` in save v13), and follow-up states; `watchman_talk`, `watchman_warned_followup`, `watchman_meal_peckish`, and `watchman_meal_tower_desc` cover tower watchman in-menu warning and meal threads, including blocked observe verbs re-showing the active menu instead of dismissing it, plus persisted tower room copy after meal `give` ([#8](https://github.com/ianmays/dosmud/issues/8), [#220](https://github.com/ianmays/dosmud/issues/220), [#235](https://github.com/ianmays/dosmud/issues/235)); the `*_modal_policy` snapshots use the new dialogue-entry fixtures to assert fixed-room parity for `bag` keep and `look` block/replay without re-running `talk`; the roaming `*_talk_blocked` snapshots now prove repeat `talk` re-shows the current roaming prompt instead of emitting a guard; peaceful roaming replies (`traveler_replies`, `lost_animal_replies`, `peddler_replies`) end with `ROOM_LOOK` after reply copy ([#235](https://github.com/ianmays/dosmud/issues/235)); `dialogue_menu_exit` uses `@fixture traveler_dialogue` and `look` to prove friendly roaming observe verbs now block and replay the current menu instead of dismissing it, matching fixed friendly dialogue; `narrative_indirection` exercises traveler, watchman, and bandit give/intimidate scenes end-to-end through the `txtres` narrative-key indirection path; [#175](https://github.com/ianmays/dosmud/pull/175), [#196](https://github.com/ianmays/dosmud/issues/196), [#205](https://github.com/ianmays/dosmud/issues/205), [#235](https://github.com/ianmays/dosmud/issues/235), [#240](https://github.com/ianmays/dosmud/issues/240), [#76](https://github.com/ianmays/dosmud/issues/76), [#132](https://github.com/ianmays/dosmud/issues/132), [#54](https://github.com/ianmays/dosmud/issues/54)).
...
**Inspect:** `inspect_rustle`, `inspect_creak`, `inspect_water`, `inspect_grit`, `inspect_none`, `inspect_wrong` (`@fixture env_focus_rustle`, `inspect creak` while `[rustles]` remains active yields lead-spent copy), `inspect_multi_clue` (`@fixture env_clues_rustle_water`, inspect each kind in sequence), `look_clues_footer` (`@fixture env_clues_rustle_water`, `look` for the compact look footer appended to the room description), `inspect_water_followup` (post-inspect water menu reply `1` follow-up copy), `env_menu_dismiss` (`@fixture env_focus_water`, `inspect`, then `north` dismisses the env menu before the move runs; deferred `ROOM_LOOK` is skipped when a roaming encounter opens on the same tick, so the snapshot shows encounter copy instead of an intermediate road look), `env_menu_blocked` (`@fixture env_focus_water`, `inspect`, then `look` to prove world-directed verbs now block and replay the active env menu instead of redrawing the room). Active clues compress into plain-language footer phrases (`txtres_look_clue_phrase`) with weather and night copy in the same footer when present. Parser accepts singular and plural inspect targets (`rustles`, `creaks`, `waters`). Atmosphere rolls announce a clue kind only the first time that bit is set in the room; later `wait`/`take` ticks keep the clue without reprinting it until `look`. `unit_game.c` (`game_move_arrival_suppresses_weather_in_room_look_footer`) and `unit_gout.c` (`game_event_push_records_room_look_flags`) assert `GAME_ROOM_LOOK_FLAG_SUPPRESS_WEATHER` when a move tick queues weather transition plus trailing `ROOM_LOOK`.

**Ambient (tick-driven):** `ambient_rustle`, `ambient_tick_order`, `ambient_item` (`@fixture ambient_camp` plus `@seed` for deterministic atmosphere, animal noise, and nearby-item paths; stack-tier tick flavor merges into one sentence before the next non-flavor event ([#236](https://github.com/ianmays/dosmud/issues/236))); `weather_rain_ambient` (`@fixture weather_rain_ready`, one `wait` for rain transition copy); `loot_menu_weather_rain` (`@fixture corpse_loot_weather_rain`, bare `loot` opens the corpse menu without advancing tick or firing the pending rain transition; menu-vs-atmosphere scene break is covered by `unit_rend.c` `render_player_corpse_menu_adds_gap_before_look_flavor`; [#206](https://github.com/ianmays/dosmud/issues/206)); `weather_rain_move_arrival` (`@fixture weather_rain_ready_quiet`, `move north` for move arrival copy that keeps the same-tick rain transition visible); `weather_rain_bandit_road` (`@fixture weather_rain_bandit_road`, `wait` on the road so the rain transition still prints before the opening bandit encounter with no trailing `ROOM_LOOK`); `weather_fog_look` (`@fixture weather_fog`, `look` for fog phrase in the look footer; [#51](https://github.com/ianmays/dosmud/issues/51)); `night_look` (`@fixture night_at_camp`, `look` for night phrase in the look footer); `night_map_nightfall` (`@fixture night_at_camp`, `map` still renders the grid before a lost roll); `night_map_lost` (`@fixture night_lost`, `map` for blank disoriented map plus exits); `night_map_torch` (`@fixture night_torch_camp`, `map` for torch illumination line plus the grid; [#130](https://github.com/ianmays/dosmud/issues/130)).

**Combat:** `combat_defend`, `combat_salve`, `combat_no_salve`, `combat_invalid`, `combat_take_blocked`, `combat_victory_xp`, `level_up`, `player_defeat_recovery` (`player_defeat_recovery` covers lethal damage, cumulative-XP level regression, camp respawn, retained marsh-root, save/load, player-corpse room copy, and `loot all`; the other combat snapshots plus unit coverage in `unit_game.c` keep the self-directed combat tier (`eat`, `use`, `craft`, `drop`) aligned with the unified modal policy, while blocked verbs replay the combat menu via `combat_replay_menu` ([#206](https://github.com/ianmays/dosmud/issues/206), [#235](https://github.com/ianmays/dosmud/issues/235), [#240](https://github.com/ianmays/dosmud/issues/240))).

**Loot:** `loot_spear`, `loot_stick`, `loot_berry`, `loot_herb`, `loot_fish`, `loot_empty`, `loot_stripped`, `loot_bag_full`, `loot_multi`, `loot_menu_exit`, `loot_all_multi`, `loot_all_bag_full`, `loot_menu_weather_rain` (interactive enemy corpse menus, selective take, bulk `loot all`, multi-slot menu, full-bag retry paths, `loot_menu_exit` dismisses the corpse menu via a non-menu explore verb before that verb runs, and `loot_menu_weather_rain` proves quiet `loot` does not advance a pending weather transition; player-corpse recovery is covered by `player_defeat_recovery` under Combat / Save/load; [#205](https://github.com/ianmays/dosmud/issues/205), [#206](https://github.com/ianmays/dosmud/issues/206)).

**Bandit dialogue:** `bandit_fight`, `bandit_intimidate_ok`, `bandit_intimidate_fail`, `bandit_bag_empty`, `bandit_road` (`bandit_intimidate_ok` and `bandit_handover` peaceful exits end with `ROOM_LOOK`; `bandit_road` skips ambient on the encounter-open tick ([#235](https://github.com/ianmays/dosmud/issues/235))).
...
**Save/load:** `save_load`, `herbalist_save_load`, `player_defeat_recovery` (single-slot `save.dat` round-trip with a deterministic post-load move, authored-story follow-up state, or corpse run; `unit_save.c` round-trip fixtures assert save v20 player-corpse presence/room/count/items ([#206](https://github.com/ianmays/dosmud/issues/206)), v19 `env_room_clues[CFG_ROOM_MAX]` per-room inspect clue bits ([#234](https://github.com/ianmays/dosmud/issues/234)), v18 `day_phase` / `day_expires_tick` / `night_lost` ([#130](https://github.com/ianmays/dosmud/issues/130)), v17 roster width (`CFG_NPC_MAX` 8), v16 `weather_kind` / `weather_expires_tick`, v15 `env_interact_*`, v14 `world_adv_flags`, v13 `herbalist_menu`, and v12 `watchman_flags` / `watchman_menu`; v19 and older files reject with `SAVE_RESULT_FORMAT`).
...
When you add or remove `src\*.c` files, update `Makefile` (`SRC` or `TEST_SRC`) and `build.bat`. For the Open Watcom path, keep every `wcl` and `wlib` line under the COMMAND.COM length limit (about 127 characters): gameplay sources are packed into `gameplay.lib` via several short `wlib` calls; the final `wcl` link lists `main.obj`, `platdos.obj`, `gameplay.lib`, plus the other `.obj` files. `build.bat` compiles the DOS path with `-os` so the release renderer and gameplay code stay under the 16-bit `_TEXT` / `AUTO` 64k linker ceiling. The DOS build compiles `replay.c` unconditionally and archives `replay.obj` into `gameplay.lib`; it never compiles the test harness (`tests/harness/`). Use `goto` labels in `build.bat` for conditionals; parenthesized `if (...)` blocks break under COMMAND.COM.
