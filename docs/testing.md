...
**Combat:** `combat_defend`, `combat_salve`, `combat_no_salve`, `combat_invalid`, `combat_take_blocked`, `combat_victory_xp`, `level_up` (`combat_salve` plus the combat unit coverage in `unit_game.c` keep the self-directed combat tier (`eat`, `use`, `craft`, `drop`) aligned with the unified modal policy, while blocked verbs replay the combat menu via `combat_replay_menu` ([#235](https://github.com/ianmays/dosmud/issues/235), [#240](https://github.com/ianmays/dosmud/issues/240))).
