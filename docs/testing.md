...
**Combat:** `combat_defend`, `combat_salve`, `combat_no_salve`, `combat_invalid`, `combat_take_blocked`, `combat_victory_xp`, `level_up` (`combat_salve` and inventory `eat` / `use` stay allowed during combat modal; blocked verbs replay the combat menu via `combat_replay_menu` ([#235](https://github.com/ianmays/dosmud/issues/235))).
