...
- `make test-run`: scripted input regression pass (`tests/smoke.*`, `tests/bandit_handover.*`, `tests/bandit_wielded_give.*`, `tests/area_items.*`, `tests/map.*`, `tests/equipment.*`, `tests/craft_wielded.*` covers wield, bag plus wielding line, unwield back to bag, first combat attack under `TEST_MODE` RNG, and crafting with a wielded ingredient)
