...
**Inspect:** `inspect_rustle`, `inspect_creak`, `inspect_water`, `inspect_grit`, `inspect_none`, `inspect_wrong` (`@fixture env_focus_rustle`, `inspect creak` names a clue kind absent from the room and gets the same "nothing stands out" copy as no active clue; wrong-focus copy was removed with the tick-scoped focus, [#234](https://github.com/ianmays/dosmud/issues/234)), `inspect_multi_clue` (`@fixture env_clues_rustle_water`, sequential no-argument `inspect` resolves each remaining single clue in the room until none remain, [#234](https://github.com/ianmays/dosmud/issues/234)), `inspect_water_followup` (post-inspect water menu reply `1` follow-up copy), `env_menu_dismiss` (`@fixture env_focus_water`, `inspect`, then `north` dismisses the env menu before the move runs; [#7](https://github.com/ianmays/dosmud/issues/7), [#205](https://github.com/ianmays/dosmud/issues/205)-style menu exit).
