...
`GameState` is the primary simulation container. Gameplay systems should mutate it explicitly and avoid shadow copies. Session fields include `env_focus_*` (tick-scoped ambient focus) and `env_interact_*` (post-inspect menu; save v15+). In `TEST_MODE` builds only, it may include the roll-inject queue and `test_quiet_ticks`; release `GameState` has neither.
