...
- **`grendr`:** use the tier helpers in [`grendr.c`](https://github.com/ianmays/dosmud/blob/main/src/grendr.c):
  ...
  - **Scene** - room look art uses `render_gap()` before the art block; character portraits and encounter opens always print one `render_gap()` before art; portraits and roaming NPC talk scenes also print one before body copy; `render_copy()` passes tight copy without an extra gap.
  ...
  - **Corpse menu then atmosphere** - `GAME_EVENT_CORPSE_VIEW` arms a deferred `render_gap` before the next look, flavor, or inspect consumer, so corpse menus stay visually separate from weather/atmosphere even when a transition folds into a later `ROOM_LOOK` ([#206](https://github.com/ianmays/dosmud/issues/206)). No other event (wait, item, craft, equip) arms this gap.
  - **Look talk hint** - when a room look prints `TXT_UI_NPC_HINT`, it follows exits / ground / corpse lines without an extra spacer.
...
- structural limits (`CFG_ROOM_MAX`, `CFG_BAG_MAX`, `CFG_AREA_ITEM_SLOTS` ground slots per room, `CFG_CORPSE_ITEM_SLOTS` per-room enemy-corpse loot slots, `CFG_PLAYER_CORPSE_ITEM_SLOTS` for one full bag plus equipped item, buffers, etc.)
- gameplay tuning (combat enemy level scaling, bandit corpse loot thresholds, food/salve heal amounts, progression including `CFG_PLAYER_DEFEAT_XP_PERCENT`, ambient systems, friendly roaming NPC timing)
...
- `game_process_input` advances the world only when `command_advances_time` is true and the step did not emit `GAME_EVENT_PLAYER_DEFEAT`; quiet `CMD_LOOT` (menu open and replies) therefore does not fire pending weather or day/night transitions ([#206](https://github.com/ianmays/dosmud/issues/206))
...
- headless step surface: `game_describe_current_room`, `game_process_input`, and `game_background_step` mutate `GameState` and append `GameEvent` records supplied by the caller; `game_describe_current_room` enqueues `ROOM_LOOK` for startup, load, and peaceful encounter / roaming reply exits; `do_look` sets `GAME_ROOM_LOOK_FLAG_SUPPRESS_WEATHER` when weather began on the current tick or the queue already holds a weather transition, and `mark_room_look_weather_suppressed` retrofits that flag onto earlier `ROOM_LOOK` snapshots queued before `advance_world_tick` on move/wait
...
- [`gprog.c`](https://github.com/ianmays/dosmud/blob/main/src/gprog.c) - cumulative XP conversion, XP and level-up rewards, and defeat XP loss plus level-derived stat rebuild (`game_xp_to_next_level`, `progression_cumulative_xp`, `progression_rebuild_from_cumulative_xp`, `progression_apply_defeat_penalty`, `progression_gain_xp`, `progression_enemy_xp_reward`, `progression_gain_enemy_xp`); queues `GAME_EVENT_XP_GAIN` and `GAME_EVENT_STAT_CHANGE` via `gout` (FAT 8.3-safe basename)
...
`game` stays orchestration; new behaviour should land in the owning slice above rather than re-centralising into `game.c`. [`game_handle_player_defeat`](https://github.com/ianmays/dosmud/blob/main/src/game.c) is the intentional cross-slice exception for [#206](https://github.com/ianmays/dosmud/issues/206) (invent and gprog mutate first; camp restore and `GAME_EVENT_PLAYER_DEFEAT` stay in `game`). [`game_heal_player`](https://github.com/ianmays/dosmud/blob/main/src/game.c) applies capped HP heals and is used by inventory eat/salve and combat salve reply 3.
...
- `command_advances_time` is true only for `wait`, `move`, `take`, `drop`, `eat`, `use`, and `craft`; `CMD_LOOT` is quiet so corpse menu open and replies do not advance weather or day/night ([#206](https://github.com/ianmays/dosmud/issues/206))
...
- room look and `GAME_EVENT_ENVIRONMENT`: `GAME_EVENT_ROOM_LOOK` arg1 packs enemy corpse (bit 0), snapshotted `weather_kind` (bits 1-2), and `day_phase` (bit 3) at enqueue time ([#51](https://github.com/ianmays/dosmud/issues/51), [#130](https://github.com/ianmays/dosmud/issues/130)); arg2 carries the room's `env_room_clues` bitmask ([#234](https://github.com/ianmays/dosmud/issues/234)); arg3 carries `GameEventRoomLookFlags` (`SUPPRESS_WEATHER`, `PLAYER_CORPSE`; [#206](https://github.com/ianmays/dosmud/issues/206)); weather, night, and active clue phrases fold into one look footer appended to the room description; clue kinds owned by the footer skip standalone `ENVIRONMENT` render when a later look will show them, but weather transitions still render standalone when that look has `SUPPRESS_WEATHER` (same-tick move/wait arrival); stack-tier tick flavor (gust, clue primaries, weather/night transitions, and related lines) batches through `atmo_stack` before flush; `render_exploration_map` consults `gatmos_night_map_blanked` (`night_lost` without torch) for blank map copy plus current-room exits, `gatmos_night_torch_lights_map` for torch illumination copy, then the normal grid
- newline tiers (`render_gap`, `atmo_stack`, look footer, and related rules): [Newline and spacing](#newline-and-spacing); one shared layout across all toolchains (no `__WATCOMC__` presentation forks)
...
- per-room enemy corpse loot (`corpse_present[]`, dense `corpse_item[][]` capped by `CFG_CORPSE_ITEM_SLOTS`); bandit victories can leave a stripped body or up to three loot items
- one globally unique player corpse (`player_corpse_*` on `GameState`, `CFG_PLAYER_CORPSE_ITEM_SLOTS`); defeat transfer and replace semantics match the player-defeat ownership paragraph under [Engine layers](#engine-layers-core--render--platform) ([#206](https://github.com/ianmays/dosmud/issues/206))
- `loot` opens `DIALOGUE_LOOT` through `GAME_EVENT_CORPSE_VIEW` (`GameEventCorpseKind` distinguishes enemy vs player); numbered take/leave replies and `loot all` drain the selected corpse in order (menu re-queues on bag-full) without advancing world time; player corpse wins when both kinds share a room
...
- wield/unwield commands track `weapon_equipped` on `GameState`; a wielded weapon is not stored in `bag[]` (it occupies the hand slot only until unwield, drop, bandit handover, or player-defeat transfer moves it)
...
- `item_is_retained_on_defeat` keep-list for player-defeat transfers (marsh-root only today; invent consults it when splitting bag and weapon into the player corpse; [#206](https://github.com/ianmays/dosmud/issues/206))
