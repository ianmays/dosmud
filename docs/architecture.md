...
- [`npc.c`](https://github.com/ianmays/dosmud/blob/main/src/npc.c) - fixed NPC identity seam, shared dialogue helpers, room look hint ownership, and the fixed-size NPC instance roster for authored fixed encounters plus roaming placement (`npc_clear_all`, `npc_spawn`, `npc_place`, `npc_move`, `npc_find_*`, `npc_begin_encounter`, `npc_end_encounter`, `npc_deactivate_until`, `npc_seed_fixed_enemies`, `npc_fixed_begin_encounter_in_room`, `npc_roaming_*`, `npc_seed_roaming_traveler`; slot order is save/tick-stable)
