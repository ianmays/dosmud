...
- versioned, field-by-field save format (`DMSV`, version 11) for `GameState`, `World`, and tracked RNG draw count
- loads only the current `SAVE_VERSION`; version 11 persists the Herbalist story slice state and `watchman_story` ([#8](https://github.com/ianmays/dosmud/issues/8)); older files return `SAVE_RESULT_FORMAT` during active development instead of carrying migration paths; successful loads use the persisted roster, combat snapshot, and authored-story room copy as written (no profile upgrade or combat backfill on read)
