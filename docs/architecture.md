...
- non-enemy dialogue menus and corpse loot menus (`DIALOGUE_LOOT`) dismiss before mode guards when the player types an explore verb that is not menu-native (`reply`, `help`, `version`, `quit`; `talk` while in non-enemy dialogue; repeat `loot` while the loot menu is open); dismissal queues `GAME_DIALOGUE_GUARD` with `GAME_DIALOGUE_GUARD_DIALOGUE_CLOSED` for dialogue menus or routes through invent leave for loot, then the verb runs in explore mode ([#205](https://github.com/ianmays/dosmud/issues/205), [#235](https://github.com/ianmays/dosmud/issues/235))
- modal encounter command policy in `game_cmd_allowed_in_mode` ([#235](https://github.com/ianmays/dosmud/issues/235)): combat allows `reply`, bag/wield maintenance, `eat` / `use` consumables, and session verbs; other commands queue `GAME_DIALOGUE_GUARD_BANDIT_WAITING_REPLY` plus `combat_replay_menu` (`GAME_COMBAT_PHASE_MENU`). Enemy bandit dialogue mirrors combat allowances for bag/wield and eat/use; blocked verbs queue the matching bandit guard and `genc_replay_active_prompt` (open or handover copy). Friendly roaming now follows the same non-enemy dismissal path as fixed dialogue for explore verbs instead of adding its own observe-only guard. Peaceful encounter resolution (`npc_roaming_cmd_reply`, bandit intimidate success, bandit give) enqueues `ROOM_LOOK` through `game_describe_current_room` after encounter copy
