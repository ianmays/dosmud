...
- command routing
- world update sequencing in `advance_world_tick`: increment tick and run `gatmos_weather_tick` first, then roster co-location encounter before roaming steps (gated by `gatmos_weather_blocks_roaming_encounter` during fog), ambient events, then non-roaming fixed slots; move and wait share the same order; no player-site random bandit roll on tick
...
- room look and `GAME_EVENT_ENVIRONMENT`: persistent weather HUD lines from `weather_kind` ([#51](https://github.com/ianmays/dosmud/issues/51)); transition subkinds `GAME_ENV_EVENT_WEATHER_*` use paragraph-tier atmosphere copy
