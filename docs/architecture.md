...
Player defeat keeps the ownership split procedural and explicit. `combat` detects lethal damage and delegates to `game_handle_player_defeat`; `invent` replaces the globally unique player corpse and transfers non-retained bag items in stable order followed by the equipped item; `items` owns the retained-on-defeat policy (currently marsh-root); and `gprog` removes 20 percent of cumulative XP before rebuilding all level-derived stats from the resulting level. The game transition moves the player to camp, restores full recalculated HP, clears player modal state, and deliberately skips the normal command tick so the victorious NPC remains active in the defeat room without an extra RNG draw. Player-corpse storage is separate from per-room enemy corpses, but `invent` reuses the same paged numbered menu and `loot all` flow, choosing the player corpse first if both corpse kinds occupy one room. A later defeat replaces the previous player corpse and discards its remaining contents.
