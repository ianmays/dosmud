...
- **`grendr`:** use the tier helpers in [`grendr.c`](https://github.com/ianmays/dosmud/blob/main/src/grendr.c):
  ...
  - **Scene** - room look art uses `render_gap()` before the art block; character portraits and encounter opens always print one `render_gap()` before art, then another before body copy; `render_copy()` passes tight copy without an extra gap.
