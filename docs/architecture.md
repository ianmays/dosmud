...
- loads only the current `SAVE_VERSION`; version 10 persists the Herbalist story slice state and older files return `SAVE_RESULT_FORMAT` during active development instead of carrying migration paths; successful loads use the persisted roster, combat snapshot, and authored-story room copy as written (no profile upgrade or combat backfill on read)
