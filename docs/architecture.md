...
- command routing
- world update sequencing in `advance_world_tick`: increment tick, run `gatmos_weather_tick`, then `gatmos_daynight_tick` ([#130](https://github.com/ianmays/dosmud/issues/130)), then roster maintenance, roaming encounter opens (fog blocks opens only, not movement), roaming steps when no encounter opened yet, fixed-slot encounter opens, then `world_step`; when co-located roaming or fixed-slot checks set `encounter_opened`, ambient rolls are skipped for that tick so `MOVE` plus encounter copy stay ahead of deferred `ROOM_LOOK` ([#235](https://github.com/ianmays/dosmud/issues/235)); move and wait share the same order; no player-site random bandit roll on tick
...
- headless step surface: `game_describe_current_room`, `game_process_input`, and `game_background_step` mutate `GameState` and append `GameEvent` records supplied by the caller; `game_describe_current_room` enqueues `ROOM_LOOK` for startup, load, and peaceful encounter exits without advancing a tick
...
- non-enemy room-NPC dialogue menus and corpse loot menus (`DIALOGUE_LOOT`) dismiss before mode guards when the player types an explore verb that is not menu-native (`reply`, `help`, `version`, `quit`; `talk` while in room-NPC dialogue; repeat `loot` while the loot menu is open); dismissal queues `GAME_DIALOGUE_GUARD` with `GAME_DIALOGUE_GUARD_DIALOGUE_CLOSED` for room-NPC dialogue or routes through invent leave for loot, then the verb runs in explore mode ([#205](https://github.com/ianmays/dosmud/issues/205)); friendly roaming encounters (traveler, lost animal, peddler via `npc_is_roaming_friendly_dialogue`) skip #205 auto-close for `look` / `inspect` / `map` and stay modal through `game_cmd_allowed_in_mode` instead ([#235](https://github.com/ianmays/dosmud/issues/235))
- modal encounter command policy in `game_cmd_allowed_in_mode` ([#235](https://github.com/ianmays/dosmud/issues/235)): combat allows `reply`, bag/wield maintenance, `eat` / `use` consumables, and session verbs; other commands queue `GAME_DIALOGUE_GUARD_BANDIT_WAITING_REPLY` plus `combat_replay_menu` (`GAME_COMBAT_PHASE_MENU`). Enemy bandit dialogue mirrors combat allowances for bag/wield and eat/use; blocked verbs queue the matching bandit guard and `genc_replay_active_prompt` (open or handover copy). Friendly roaming blocks `look` / `inspect` / `map` with `GAME_DIALOGUE_GUARD_ROAMING_ENCOUNTER_WAITING` plus `npc_roaming_replay_scene`. Peaceful encounter resolution (`npc_roaming_cmd_reply`, bandit intimidate success, bandit give) enqueues `ROOM_LOOK` through `game_describe_current_room` after encounter copy
...
- `GAME_EVENT_ENV_MENU` and `GAME_EVENT_ENV_RESULT`: post-inspect ambient follow-up menus and numbered reply outcomes ([#7](https://github.com/ianmays/dosmud/issues/7)); `GAME_EVENT_DIALOGUE_GUARD` reasons include bandit waiting/handover guards, loot waiting, `GAME_DIALOGUE_GUARD_ROAMING_ENCOUNTER_WAITING` when observe verbs are blocked during friendly roaming encounters ([#235](https://github.com/ianmays/dosmud/issues/235)), `GAME_DIALOGUE_GUARD_DIALOGUE_CLOSED` when #205 dismisses a room-NPC menu, and `GAME_DIALOGUE_GUARD_ENV_MENU_CLOSED` when an explore verb dismisses the post-inspect menu before running
