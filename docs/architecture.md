...
- **`grendr`:** use the tier helpers in [`grendr.c`](https://github.com/ianmays/dosmud/blob/main/src/grendr.c):
  ...
  - **25-line safety guard** - handled command frames target `CFG_SAFE_OUTPUT_MAX_LINES` visible rows (step output plus HUD, prompt excluded). In `TEST_MODE`, `grendr` counts emitted newline rows for the frame and `main.c` fails fast if the frame exceeds the cap.
