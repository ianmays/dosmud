...
Modules include `game`, `command`, `world`, `invent`, `combat`, `gatmos`, `gprog`, `gstory`, `genc`, `dialogue`, `npc`, and `items`.
...
- **Game logic** - dosmud-specific rules and content that plug into that stepping surface: command routing in `game`, room/world rules, and the gameplay slices (`combat`, `invent`, `dialogue`, `npc`, `genc`, `gatmos`, `gprog`, `gstory`, `items`).
...
- versioned, field-by-field save format (`DMSV`, version 12) for `GameState`, `World`, and tracked RNG draw count
- loads only the current `SAVE_VERSION`; version 12 persists the Herbalist story slice state, composable `watchman_flags`, and session `watchman_menu` ([#8](https://github.com/ianmays/dosmud/issues/8)); older files return `SAVE_RESULT_FORMAT` during active development instead of carrying migration paths; successful loads use the persisted roster, combat snapshot, and authored-story room copy as written (no profile upgrade or combat backfill on read)
...
- [`npc.c`](https://github.com/ianmays/dosmud/blob/main/src/npc.c) - fixed NPC identity seam, shared dialogue helpers, room look hint ownership, the parallel `NPC_ROOM_INFO[]` room-talk table (talk opens by player `room_id`, reply resolves by `game.dialogue` through `npc_open_room_dialogue` / `npc_room_cmd_reply`; `give` / `offer` to room NPCs route through `npc_cmd_give` only when enemy handover does not own the command; kept separate from `NPC_PROFILES[]` per [#197](https://github.com/ianmays/dosmud/issues/197)), the narrow Herbalist authored-story slice for [#76](https://github.com/ianmays/dosmud/issues/76) plus the reusable authored item-exchange / reward seam from [#132](https://github.com/ianmays/dosmud/issues/132) (story transitions, orchard desc mutation, reply routing; fetch-quest scene derivation and marsh-root seeding delegate to `gstory` per [#49](https://github.com/ianmays/dosmud/issues/49); accepted/rejected give feedback, npc-owned pre-removal bag/full-ground planning, invent-owned carried-item removal plus salve reward writes, completed Orchard room-copy hook), the tower watchman authored branch for [#8](https://github.com/ianmays/dosmud/issues/8) (`WatchmanStoryState` / `WatchmanDialogueScene`; warned follow-up talk, meal-offer herb grant to bag or ground, and reply copy keyed through `GAME_EVENT_DIALOGUE` `arg3`), and the fixed-size NPC instance roster for authored enemy profiles plus traveler roaming (`npc_clear_all`, `npc_spawn`, `npc_place`, `npc_move`, `npc_find_*`, `npc_begin_encounter`, `npc_end_encounter`, `npc_deactivate_until`, `npc_seed_profiles`, `npc_fixed_begin_encounter_in_room`, `npc_roaming_*`; `NPC_PROFILES[]` holds traveler and three roaming bandit profiles (road, bridge, canyon) with authored `level_min`/`level_max`, roam warmup, and respawn timing; bandits open through `npc_roaming_begin_encounter_in_room` and respawn via `npc_end_encounter`; `npc_fixed_begin_encounter_in_room` remains for future non-roaming authored slots; slot order is save/tick-stable)
...
New `src/*.c` and `src/*.h` basenames must stay within **classic FAT 8+3** (at most eight characters before `.c` or `.h`) so MS-DOS 5.x-6.x style volumes and the Open Watcom DOS build can open them reliably. Existing examples: `grendr.*`, `invent.*`, `gprog.*`, `gstory.*`, `gatmos.*`, `genc.*`.
...
- `GAME_EVENT_DIALOGUE` and `GAME_EVENT_ENCOUNTER`: resolve `TxtresNarrativeKey` via `txtres`, then call the portrait/menu or encounter `render_*` helper for that key; `GAME_EVENT_DIALOGUE` `arg3` carries authored scene detail when talk/reply/give branches need more than choice alone (Herbalist story and exchange scenes for [#76](https://github.com/ianmays/dosmud/issues/76) and [#132](https://github.com/ianmays/dosmud/issues/132); watchman in-menu talk menus (neutral, after-warning, meal-offer, after-meal) and reply outcomes for [#8](https://github.com/ianmays/dosmud/issues/8))
