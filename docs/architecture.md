...
- gameplay tuning (combat and bandit corpse loot thresholds, food/salve heal amounts, progression, ambient systems, bandit/traveler roaming timing)
...
- [`npc.c`](https://github.com/ianmays/dosmud/blob/main/src/npc.c) - fixed NPC identity seam, shared dialogue helpers, room look hint ownership, and the fixed-size NPC instance roster for authored enemy profiles plus traveler roaming (`npc_clear_all`, `npc_spawn`, `npc_place`, `npc_move`, `npc_find_*`, `npc_begin_encounter`, `npc_end_encounter`, `npc_deactivate_until`, `npc_seed_fixed_enemies`, `npc_fixed_begin_encounter_in_room`, `npc_roaming_*`, `npc_seed_roaming_traveler`; the road bandit seeds on `WORLD_ROOM_ROAD` with `NPC_FLAG_ROAMING | NPC_FLAG_RESPAWNS`, opens through `npc_roaming_begin_encounter_in_room`, and respawns via `npc_end_encounter`; `npc_fixed_begin_encounter_in_room` remains for future non-roaming authored slots; slot order is save/tick-stable)
